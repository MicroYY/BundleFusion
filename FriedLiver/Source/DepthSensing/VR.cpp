...
extern "C" void colorWithPointCloudRayCast(uchar4* d_output, const uchar4* d_input1, float4* d_input2, unsigned int width, unsigned int height);
...
void __VR_runner(CUDAImageManager* g_CudaImageManager, CUDARayCastSDF* g_rayCast, const uchar4* image)
{
	...
	int winWidth = g_CudaImageManager->getIntegrationWidth();
	int winHeight = g_CudaImageManager->getIntegrationHeight();
	...
	int zedWidth = g_CudaImageManager->getIntegrationWidth();
	int zedHeight = g_CudaImageManager->getIntegrationHeight();
	...
	uchar4* blend;
	cudaMalloc(&blend, sizeof(uchar4) * 640 * 480);
	...
	while (!end) {
		...
		if (isVisible) {
			...
			if (1) {
				...
				//colorWithPointCloudRayCast(blend, g_CudaImageManager->getLastIntegrateFrame().getColorFrameGPU(), g_rayCast->getRayCastData().d_depth4, 640, 480);
				cudaMemcpy2DToArray(arrIm, 0, 0, image, 640 * 4 * 1, 640 * 4 * 1, 480, cudaMemcpyDeviceToDevice);
				...
				cudaMemcpy2DToArray(arrIm, 0, 0, image, 4 * 640 * 1, 640 * 4 * 1, 480, cudaMemcpyDeviceToDevice);
