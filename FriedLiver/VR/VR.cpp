...
int main(int argc, char** argv)
{
	...
	HANDLE hMapFileImage;
	LPVOID lpbase;
	hMapFileImage = OpenFileMapping(FILE_MAP_READ, FILE_MAP_READ, TEXT("imageData"));
	lpbase = MapViewOfFile(hMapFileImage, FILE_MAP_READ, 0, 0, 0);
	cv::Mat image3 = cv::Mat(480, 640, CV_8UC3);
	cv::Mat image4 = cv::Mat(480, 640, CV_8UC4);
	...
	std::cout << std::endl << "Starting mapping process.." << std::endl;
	TCHAR mappingProcess[] = TEXT("x64\\Release\\Mapping.exe");
	STARTUPINFO si = { 0 };
	PROCESS_INFORMATION pi;
	DWORD dwExitCode;
	auto iRet = CreateProcess(mappingProcess, NULL, NULL, NULL, false, NULL, NULL, NULL, &si, &pi);
	if (iRet)
	{
	...
	while (!end) {
		...
		if (isVisible) {
			...
			if (1) {
				...
				//memcpy(image3.data, lpbase, sizeof(uchar) * 4 * 640 * 480);
				image3.data = (uchar*)lpbase;
				cv::cvtColor(image3, image4, cv::COLOR_RGB2RGBA);
				...
				cudaMemcpy2DToArray(arrIm, 0, 0, image4.data, 640 * 4 * 1, 640 * 4 * 1, 480, cudaMemcpyHostToDevice);
				...
				cudaMemcpy2DToArray(arrIm, 0, 0, image4.data, 4 * 640 * 1, 640 * 4 * 1, 480, cudaMemcpyHostToDevice);
